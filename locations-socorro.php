<?php
/**
 * Template Name: Location - Socorro
 */

get_header(); ?>

<main class="bg-white text-[#42474b] overflow-hidden">

  <!-- HERO + H1 -->
  <section class="relative overflow-hidden bg-[linear-gradient(135deg,#132d41_0%,#1c425f_100%)] py-20 text-white">
    <div class="mx-auto max-w-5xl px-4 text-center">

      <p class="ajs-reveal text-xs font-black uppercase tracking-[0.18em] text-[#6eaace]">
        Socorro
      </p>

      <h1 class="ajs-reveal mt-4 text-4xl font-black leading-tight tracking-[-0.04em] md:text-5xl lg:text-6xl">
        Roofing Company in Socorro · AJS Roofing &amp; Gutters
      </h1>

      <div class="ajs-reveal mt-8 flex flex-col items-center justify-center gap-4 sm:flex-row">
        <a href="/estimate" class="ajs-btn-primary">Get Your Free Inspection</a>
        <a href="tel:+15054535626" class="ajs-btn-secondary">Call (505) 453-5626</a>
      </div>

    </div>
  </section>

  <!-- ENTITY STATEMENT -->
  <section class="py-16 md:py-20">
    <div class="mx-auto max-w-4xl px-4">

      <h2 class="ajs-reveal text-3xl font-black tracking-[-0.03em] text-[#132d41] md:text-4xl">
        Where Chaos Ends. Home Begins.
      </h2>

      <p class="ajs-reveal mt-6 text-lg leading-8 text-[#42474b]">
        AJS Roofing &amp; Gutters is a licensed roofing contractor serving Socorro and the surrounding area, helping homeowners move from uncertainty to clarity with a process built on communication, craftsmanship, and protection that lasts. We don't just install roofs, we restore peace of mind. Through clear recommendations, professional execution, and work done right from the start, we help homeowners feel confident in every step of the project.
      </p>

      <p class="ajs-reveal mt-5 text-lg leading-8 text-[#42474b]">
        With manufacturer certifications and our own on-site gutter fabrication machine, AJS is the roofing partner that brings order to a process that usually feels chaotic with recommendations built for what New Mexico's UV, hail, wind, and monsoon conditions actually do to a roof. [PLACEHOLDER: Add a Socorro-specific line about local housing, storm patterns, or common roofing conditions.]
      </p>

    </div>
  </section>

  <!-- LOCAL HOUSING DATA -->
  <section class="bg-[#f5f7f8] py-16 md:py-20">
    <div class="mx-auto max-w-5xl px-4 text-center">

      <p class="ajs-reveal mb-3 text-xs font-black uppercase tracking-[0.18em] text-[#d04418]">
        The Socorro Market
      </p>

      <h2 class="ajs-reveal text-3xl font-black tracking-[-0.03em] text-[#132d41]">
        Roofing Demand in Socorro Requires Clear, Reliable Solutions.
      </h2>

      <div class="mt-10 flex justify-center">
        <div class="grid w-full max-w-2xl gap-6 md:grid-cols-2">

          <div class="ajs-stat-card ajs-reveal text-center">
            <span class="ajs-stat-number block">Placeholder</span>
            <span class="ajs-stat-label block mt-2">
              Add local market insight here
            </span>
          </div>

          <div class="ajs-stat-card ajs-reveal text-center">
            <span class="ajs-stat-number block">Placeholder</span>
            <span class="ajs-stat-label block mt-2">
              Add dominant roof type or demand trend
            </span>
          </div>

        </div>
      </div>

    </div>
  </section>

  <!-- SERVICES -->
  <section class="py-16 md:py-20">
    <div class="mx-auto max-w-7xl px-4">

      <p class="ajs-reveal mb-3 text-center text-xs font-black uppercase tracking-[0.18em] text-[#d04418]">
        Protection You Can Feel
      </p>

      <h2 class="ajs-reveal text-center text-3xl font-black tracking-[-0.03em] text-[#132d41]">
        Roofing &amp; Gutter Services in Socorro
      </h2>

      <div class="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">

        <a href="/service/roof-replacement" class="ajs-location-card ajs-reveal">
          <span class="ajs-card-title">Roof Replacement</span>
          <span class="ajs-card-desc">Complete tear-off and rebuild. Premium materials, manufacturer warranties, and our documented process. Done right. Done once.</span>
        </a>

        <a href="/service/metal-roofing" class="ajs-location-card ajs-reveal">
          <span class="ajs-card-title">Metal Roofing</span>
          <span class="ajs-card-desc">Standing seam and metal panel systems built for New Mexico's extreme UV, hail, and wind. 40–70 year lifespan. The last roof your home will need.</span>
        </a>

        <a href="/service/gutters" class="ajs-location-card ajs-reveal">
          <span class="ajs-card-title">Seamless Gutters</span>
          <span class="ajs-card-desc">Custom-fabricated on-site with our own machine. No seams, no leaks, no subcontractors. Precision drainage that protects your foundation.</span>
        </a>

        <a href="/service/roof-repair" class="ajs-location-card ajs-reveal">
          <span class="ajs-card-title">Roof Repair</span>
          <span class="ajs-card-desc">Honest assessment first we'll tell you if a repair is enough or if replacement makes more sense.</span>
        </a>

        <a href="/service/storm-damage-insurance" class="ajs-location-card ajs-reveal">
          <span class="ajs-card-title">Storm Damage &amp; Insurance</span>
          <span class="ajs-card-desc">We inspect, document everything with photos, and work with your insurance to streamline the process.</span>
        </a>

        <a href="/service/roof-inspection" class="ajs-location-card ajs-reveal">
          <span class="ajs-card-title">Roof Inspection &amp; Maintenance</span>
          <span class="ajs-card-desc">Photo-documented inspections and annual maintenance plans. Catch small problems before they become expensive ones.</span>
        </a>

      </div>

    </div>
  </section>

  <!-- LOCAL PROJECTS -->
  <section class="bg-[#f5f7f8] py-16 md:py-20">
    <div class="mx-auto max-w-7xl px-4">

      <p class="ajs-reveal mb-3 text-center text-xs font-black uppercase tracking-[0.18em] text-[#d04418]">
        Installed Right. Protected for Years.
      </p>

      <h2 class="ajs-reveal text-center text-3xl font-black text-[#132d41]">
        Projects in Socorro
      </h2>

      <div class="mt-10 grid gap-5 md:grid-cols-3">

        <div class="ajs-img-card ajs-reveal">
          <img 
            src="http://ajs-roofing-and-gutters.local/wp-content/uploads/2026/03/paragon-exterior-uvuU43FAwQ-unsplash-scaled.jpg" 
            alt="Roofing project Socorro"
            class="h-full w-full rounded-[20px] object-cover"
          >
        </div>

        <div class="ajs-img-card ajs-reveal">
          <img 
            src="http://ajs-roofing-and-gutters.local/wp-content/uploads/2026/03/raze-solar-umJz4M-iCOw-unsplash-scaled.jpg" 
            alt="Roof replacement Socorro"
            class="h-full w-full rounded-[20px] object-cover"
          >
        </div>

        <div class="ajs-img-card ajs-reveal">
          <img 
            src="http://ajs-roofing-and-gutters.local/wp-content/uploads/2026/03/sergej-karpow-bquoYvs3KtQ-unsplash-scaled.jpg" 
            alt="Seamless gutters Socorro"
            class="h-full w-full rounded-[20px] object-cover"
          >
        </div>

      </div>

    </div>
  </section>

  <!-- LOCAL LOCATION -->
  <section class="bg-white py-16 md:py-20">
    <div class="mx-auto max-w-7xl px-4">
      <div class="grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">

        <div class="ajs-reveal">
          <p class="mb-3 text-xs font-black uppercase tracking-[0.18em] text-[#d04418]">
            Location
          </p>

          <h2 class="text-3xl font-black tracking-[-0.03em] text-[#132d41] md:text-4xl">
            Socorro · [ZIP PLACEHOLDER]
          </h2>

          <p class="mt-6 text-lg leading-8 text-[#42474b]">
            AJS Roofing &amp; Gutters serves homeowners across Socorro and the surrounding area with clear communication, documented processes, and reliable execution from start to finish.
          </p>

          <div class="mt-6 rounded-[18px] border border-[#d3d8db] bg-[#f5f7f8] p-5">
            <p class="text-sm font-black uppercase tracking-[0.14em] text-[#d04418]">
              Service Area
            </p>
            <p class="mt-2 text-base font-bold text-[#132d41]">
              Socorro · New Mexico · [ZIP PLACEHOLDER]
            </p>
          </div>
        </div>

        <div class="ajs-reveal">
          <div class="overflow-hidden rounded-[22px] border border-[#d3d8db] shadow-[0_14px_32px_rgba(19,45,65,0.08)]">
            <iframe
              src="https://www.google.com/maps?q=Socorro,+NM&output=embed"
              width="100%"
              height="340"
              style="border:0;"
              allowfullscreen=""
              loading="lazy"
              referrerpolicy="no-referrer-when-downgrade">
            </iframe>
          </div>
        </div>

      </div>
    </div>
  </section>

  <!-- FAQ con FAQPage schema -->
  <section class="py-16 md:py-20">
    <div class="mx-auto max-w-4xl px-4">

      <p class="ajs-reveal mb-3 text-center text-xs font-black uppercase tracking-[0.18em] text-[#d04418]">
        Common Questions
      </p>

      <h2 class="ajs-reveal text-center text-3xl font-black text-[#132d41]">
        Roofing Questions from Socorro Homeowners
      </h2>

      <div class="mt-10 space-y-4" itemscope itemtype="https://schema.org/FAQPage">

        <details class="ajs-faq ajs-reveal" itemscope itemprop="mainEntity" itemtype="https://schema.org/Question">
          <summary itemprop="name">Do I need a full roof replacement or just a repair?</summary>
          <div itemscope itemprop="acceptedAnswer" itemtype="https://schema.org/Answer">
            <p itemprop="text">That depends on the age of the roof, the extent of the damage, and what we find during the inspection. AJS starts with a photo-documented evaluation so you can clearly see whether a repair is enough or whether replacement makes more long-term sense.</p>
          </div>
        </details>

        <details class="ajs-faq ajs-reveal" itemscope itemprop="mainEntity" itemtype="https://schema.org/Question">
          <summary itemprop="name">How does New Mexico weather affect roofs in Socorro?</summary>
          <div itemscope itemprop="acceptedAnswer" itemtype="https://schema.org/Answer">
            <p itemprop="text">Roofs in Socorro deal with intense UV exposure, seasonal wind, storm activity, and monsoon moisture. Over time, that combination can accelerate wear on shingles, flashing, sealants, and drainage systems. Regular inspections help catch problems before they become costly.</p>
          </div>
        </details>

        <details class="ajs-faq ajs-reveal" itemscope itemprop="mainEntity" itemtype="https://schema.org/Question">
          <summary itemprop="name">Do you help with storm damage insurance claims in Socorro?</summary>
          <div itemscope itemprop="acceptedAnswer" itemtype="https://schema.org/Answer">
            <p itemprop="text">Yes. AJS can inspect the roof, document visible damage with photos, and help homeowners move through the insurance process with more clarity and less confusion.</p>
          </div>
        </details>

        <details class="ajs-faq ajs-reveal" itemscope itemprop="mainEntity" itemtype="https://schema.org/Question">
          <summary itemprop="name">What makes AJS different from other roofing contractors in Socorro?</summary>
          <div itemscope itemprop="acceptedAnswer" itemtype="https://schema.org/Answer">
            <p itemprop="text">AJS follows a documented process, communicates clearly, fabricates seamless gutters on-site, and focuses on solutions built for New Mexico conditions instead of generic national recommendations.</p>
          </div>
        </details>

        <details class="ajs-faq ajs-reveal" itemscope itemprop="mainEntity" itemtype="https://schema.org/Question">
          <summary itemprop="name">Do you serve all of Socorro?</summary>
          <div itemscope itemprop="acceptedAnswer" itemtype="https://schema.org/Answer">
            <p itemprop="text">Yes. AJS Roofing &amp; Gutters serves homeowners throughout Socorro and nearby communities with roofing and gutter solutions designed for long-term protection.</p>
          </div>
        </details>

      </div>

    </div>
  </section>

  <!-- CTA WITH FORM -->
  <section class="overflow-hidden bg-[#f5f7f8] py-20">
    <div class="mx-auto grid max-w-7xl gap-10 px-4 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">

      <div class="ajs-reveal-left">
        <p class="mb-3 text-xs font-black uppercase tracking-[0.18em] text-[#d04418]">
          Socorro
        </p>

        <h2 class="max-w-[14ch] text-3xl font-black leading-tight tracking-[-0.03em] text-[#132d41] md:text-5xl">
          When Everything Is Right Above, Calm Returns Home.
        </h2>

        <p class="mt-5 max-w-2xl text-base leading-8 text-[#42474b]">
          Schedule your free inspection. We'll show you exactly what your roof needs
          and exactly what it doesn't. No pressure. No surprises. Just a clear path from uncertainty to protection.
        </p>
      </div>

      <div class="ajs-reveal-right rounded-[30px] bg-white p-6 text-slate-900 shadow-[0_20px_44px_rgba(19,45,65,0.12)] border border-[#d3d8db]">

        <p class="text-xs font-black uppercase tracking-[0.18em] text-[#d04418]">
          Quick Inspection Request
        </p>

        <h3 class="mt-3 text-2xl font-black tracking-[-0.03em] text-[#132d41]">
          Tell us what you need.
        </h3>

        <div id="ajsLocationFormSuccess" class="hidden mt-5 rounded-2xl border border-green-200 bg-green-50 px-4 py-4 text-sm text-green-700">
          Thanks for reaching out. Your inspection request was sent successfully.
        </div>

        <div id="ajsLocationFormError" class="hidden mt-5 rounded-2xl border border-red-200 bg-red-50 px-4 py-4 text-sm text-red-700">
          Something went wrong. Please try again.
        </div>

        <form id="ajsLocationForm" class="mt-5 grid gap-3">

          <input id="ajs_location_name" name="name" type="text" placeholder="Full Name *" required
            class="w-full rounded-2xl border border-[#d3d8db] bg-white px-4 py-4 text-sm text-[#132d41] placeholder:text-[#42474b]/65 outline-none focus:border-[#6eaace] focus:ring-4 focus:ring-[#6eaace]/20">

          <input id="ajs_location_phone" name="phone" type="tel" placeholder="Phone Number *" required
            class="w-full rounded-2xl border border-[#d3d8db] bg-white px-4 py-4 text-sm text-[#132d41] placeholder:text-[#42474b]/65 outline-none focus:border-[#6eaace] focus:ring-4 focus:ring-[#6eaace]/20">

          <input id="ajs_location_email" name="email" type="email" placeholder="Email *" required
            class="w-full rounded-2xl border border-[#d3d8db] bg-white px-4 py-4 text-sm text-[#132d41] placeholder:text-[#42474b]/65 outline-none focus:border-[#6eaace] focus:ring-4 focus:ring-[#6eaace]/20">

          <select id="ajs_location_service" name="service" required
            class="w-full rounded-2xl border border-[#d3d8db] bg-white px-4 py-4 text-sm text-[#132d41] outline-none focus:border-[#6eaace] focus:ring-4 focus:ring-[#6eaace]/20">
            <option value="" disabled selected>Select a service</option>
            <option value="Roof Replacement">Roof Replacement</option>
            <option value="Metal Roofing">Metal Roofing</option>
            <option value="Gutters">Gutters</option>
            <option value="Roof Repair">Roof Repair</option>
            <option value="Storm Damage / Insurance">Storm Damage / Insurance</option>
            <option value="Inspection / Maintenance">Inspection / Maintenance</option>
            <option value="Not Sure">Not Sure</option>
          </select>

          <input id="ajs_location_address" name="address" type="text" placeholder="Address or ZIP Code *" required
            class="w-full rounded-2xl border border-[#d3d8db] bg-white px-4 py-4 text-sm text-[#132d41] placeholder:text-[#42474b]/65 outline-none focus:border-[#6eaace] focus:ring-4 focus:ring-[#6eaace]/20">

          <textarea id="ajs_location_message" name="message" rows="3" placeholder="Anything else we should know?"
            class="w-full resize-y rounded-2xl border border-[#d3d8db] bg-white px-4 py-4 text-sm text-[#132d41] placeholder:text-[#42474b]/65 outline-none focus:border-[#6eaace] focus:ring-4 focus:ring-[#6eaace]/20"></textarea>

          <div class="flex items-start gap-3 mt-2">
            <input id="ajs_location_consent" type="checkbox" class="mt-1 flex-shrink-0" required>
            <label for="ajs_location_consent" class="text-sm text-[#42474b]">
              I agree to be contacted by AJS Roofing &amp; Gutters regarding my request.
            </label>
          </div>

          <button
            id="ajsLocationSubmitBtn"
            type="submit"
            class="ajs-btn-animate inline-flex items-center justify-center rounded-full bg-[#d04418] px-6 py-4 text-sm font-black text-white shadow-[0_16px_32px_rgba(208,68,24,0.24)] mt-4">
            Get Your Free Inspection
          </button>

          <p class="text-center text-sm text-[#42474b]/70 mt-2">
            Response within 24 hours. Detailed photo report included.
          </p>

        </form>
      </div>

    </div>
  </section>

</main>

<style>
.hidden {
  display: none !important;
}

.ajs-btn-primary {
  background: #d04418;
  padding: 16px 38px;
  font-weight: 900;
  border-radius: 999px;
  color: #ffffff;
  font-size: 0.92rem;
  box-shadow: 0 14px 30px rgba(208, 68, 24, 0.28);
  transition: all .25s ease;
  display: inline-flex;
  align-items: center;
  justify-content: center;
  border: none;
  cursor: pointer;
}

.ajs-btn-primary:hover {
  transform: translateY(-2px);
  box-shadow: 0 18px 36px rgba(208, 68, 24, 0.36);
}

.ajs-btn-secondary {
  padding: 15px 32px;
  font-weight: 900;
  border-radius: 999px;
  color: #ffffff;
  font-size: 0.92rem;
  border: 1px solid rgba(255,255,255,0.22);
  background: rgba(255,255,255,0.08);
  transition: all .25s ease;
  display: inline-flex;
  align-items: center;
  justify-content: center;
}

.ajs-btn-secondary:hover {
  background: rgba(255,255,255,0.14);
}

.ajs-stat-card {
  padding: 24px;
  border-radius: 20px;
  border: 1px solid rgba(211, 216, 219, 0.9);
  background: #ffffff;
  box-shadow: 0 12px 28px rgba(19, 45, 65, 0.06);
  display: flex;
  flex-direction: column;
  gap: 6px;
}

.ajs-stat-number {
  font-size: 1.6rem;
  font-weight: 900;
  color: #132d41;
  letter-spacing: -0.03em;
  line-height: 1;
}

.ajs-stat-label {
  font-size: 0.78rem;
  font-weight: 800;
  text-transform: uppercase;
  letter-spacing: 0.08em;
  color: #d04418;
}

.ajs-location-card {
  padding: 22px 24px;
  border-radius: 18px;
  border: 1px solid rgba(211, 216, 219, 0.9);
  background: linear-gradient(180deg, #ffffff 0%, #f5f7f8 100%);
  transition: all .25s ease;
  display: flex;
  flex-direction: column;
  gap: 6px;
  box-shadow: 0 8px 20px rgba(19, 45, 65, 0.05);
}

.ajs-location-card:hover {
  transform: translateY(-3px);
  box-shadow: 0 16px 32px rgba(19, 45, 65, 0.1);
  border-color: rgba(110, 170, 206, 0.22);
}

.ajs-card-title {
  font-size: 1rem;
  font-weight: 900;
  color: #132d41;
}

.ajs-card-desc {
  font-size: 0.87rem;
  color: #42474b;
  font-weight: 500;
  line-height: 1.55;
}

.ajs-img-card {
  overflow: hidden;
  border-radius: 20px;
  box-shadow: 0 14px 30px rgba(19, 45, 65, 0.08);
}

.ajs-img-card img {
  width: 100%;
  height: 260px;
  object-fit: cover;
  transition: transform .6s ease;
  display: block;
}

.ajs-img-card:hover img {
  transform: scale(1.04);
}

.ajs-faq {
  border: 1px solid rgba(211, 216, 219, 0.9);
  border-radius: 16px;
  padding: 18px 20px;
  background: #ffffff;
  box-shadow: 0 4px 12px rgba(19, 45, 65, 0.04);
}

.ajs-faq summary {
  font-weight: 800;
  cursor: pointer;
  color: #132d41;
  font-size: 0.97rem;
  line-height: 1.4;
  list-style: none;
}

.ajs-faq summary::-webkit-details-marker { display: none; }
.ajs-faq[open] summary { color: #6eaace; }

.ajs-faq p {
  margin-top: 12px;
  font-size: 0.93rem;
  color: #42474b;
  line-height: 1.7;
}

.ajs-form-wrap {
  background: rgba(255,255,255,0.06);
  border: 1px solid rgba(255,255,255,0.1);
  border-radius: 24px;
  padding: 2rem;
  max-width: 720px;
  margin: 0 auto;
}

.ajs-field {
  display: flex;
  flex-direction: column;
  gap: 6px;
}

.ajs-field label {
  font-size: 0.82rem;
  font-weight: 800;
  color: rgba(255,255,255,0.75);
  text-transform: uppercase;
  letter-spacing: 0.06em;
}

.ajs-field input,
.ajs-field select,
.ajs-field textarea {
  background: rgba(255,255,255,0.08);
  border: 1px solid rgba(255,255,255,0.14);
  border-radius: 12px;
  padding: 12px 14px;
  color: #ffffff;
  font-size: 0.93rem;
  font-weight: 600;
  outline: none;
  transition: border-color .2s ease;
  width: 100%;
}

.ajs-field input::placeholder,
.ajs-field textarea::placeholder {
  color: rgba(255,255,255,0.35);
}

.ajs-field input:focus,
.ajs-field select:focus,
.ajs-field textarea:focus {
  border-color: rgba(110, 170, 206, 0.6);
}

.ajs-field select option {
  background: #132d41;
  color: #ffffff;
}

.ajs-reveal {
  opacity: 0;
  transform: translateY(24px);
  transition: opacity .7s ease, transform .7s ease;
}

.ajs-reveal.is-visible {
  opacity: 1;
  transform: translateY(0);
}
</style>

<script src="https://cdn.jsdelivr.net/npm/@emailjs/browser@4/dist/email.min.js"></script>

<script>
document.addEventListener("DOMContentLoaded", () => {
  const items = document.querySelectorAll(".ajs-reveal");
  const observer = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      if (entry.isIntersecting) entry.target.classList.add("is-visible");
    });
  }, { threshold: 0.12 });
  items.forEach(el => observer.observe(el));

  if (window.emailjs) {
    emailjs.init({
      publicKey: "fO4Vd7UFZCyOrU5J6"
    });
  }

  const form = document.getElementById("ajsLocationForm");
  const submitBtn = document.getElementById("ajsLocationSubmitBtn");
  const successBox = document.getElementById("ajsLocationFormSuccess");
  const errorBox = document.getElementById("ajsLocationFormError");

  if (form) {
    form.addEventListener("submit", function (e) {
      e.preventDefault();

      successBox.classList.add("hidden");
      errorBox.classList.add("hidden");

      const originalButtonText = submitBtn.textContent;
      submitBtn.disabled = true;
      submitBtn.textContent = "Sending...";

      const formData = {
        name: document.getElementById("ajs_location_name").value,
        phone: document.getElementById("ajs_location_phone").value,
        email: document.getElementById("ajs_location_email").value,
        service: document.getElementById("ajs_location_service").value,
        address: document.getElementById("ajs_location_address").value,
        message: document.getElementById("ajs_location_message").value || "No additional details provided.",
        consent: document.getElementById("ajs_location_consent").checked ? "Yes" : "No"
      };

      emailjs.send(
        "service_siv1hxi",
        "template_kcyfw3g",
        formData
      ).then(function () {
        form.reset();
        successBox.classList.remove("hidden");
        submitBtn.disabled = false;
        submitBtn.textContent = originalButtonText;
      }).catch(function (error) {
        console.error("EmailJS error:", error);
        errorBox.classList.remove("hidden");
        submitBtn.disabled = false;
        submitBtn.textContent = originalButtonText;
      });
    });
  }
});
</script>

<?php get_footer(); ?>